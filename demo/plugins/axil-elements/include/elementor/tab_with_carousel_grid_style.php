...
class Blogar_Elementor_Widget_Tab_with_Carousel extends Widget_Base {

    use \Elementor\BlogarElementCommonFunctions;

    public function get_name() {
        return 'blogar-tab-with-grid-style';
    }

    public function get_title() {
        return __( 'Tab Grid Style - Carousel', 'blogar' );
    }
    ...
    protected function _register_controls() {
        ...
        $this->axil_section_title('tab_with_slider_post_title', 'Innovation & Tech', 'h2','true', 'text-left');

        $this->start_controls_section(
            '_tab_with_carousel_extra',
            [
        ...
        $this->add_group_control(
            ...
            [
                'name' => 'tab_with_carousel_thumb_size',
        ...
        $this->axil_basic_style_controls('tab_with_carousel_title', 'Section Title', '.section-title .title');
        $this->axil_section_style_controls('tab_with_carousel_area', 'Area Style', '.axil-tab-area');
    ...
    protected function render( $instance = [] ) {
        ...
        $tab_with_carousel_thumb_size = (isset($settings['tab_with_carousel_thumb_size']) && !empty($settings['tab_with_carousel_thumb_size'])) ? $settings['tab_with_carousel_thumb_size'] : "axil-tab-post-thumb";
        ...
        <div class="axil-tab-area axil-section-gap bg-color-white">
            <div class="wrapper">
                <div class="container">
                    <div class="row">
                        <div class="col-lg-12">
                            <div class="section-title <?php echo esc_attr($settings['axil_tab_with_slider_post_title_align']); ?>">
                                <?php $this->axil_section_title_render('tab_with_slider_post_title',  $this->get_settings()); ?>
                            </div>
                        </div>
                    </div>
                    <div class="row">
                            ...
                            <div class="tab-content" id="axilTabContent-<?php echo esc_attr($this->get_id());?>">
                                ...
                                $j = 0; foreach($settings['tabs'] as $tab): $j++;
                                    ...
                                    <?php if ( $tab_post_query->have_posts() ) : ?>
                                        <div class="single-tab-content tab-pane fade <?php echo esc_attr(($j > 1) ? '' : 'show active'); ?>" id="tab-<?php echo esc_attr($this->get_id() . $j); ?>" role="tabpanel" aria-labelledby="tab-<?php echo esc_attr($this->get_id() . $j); ?>">
                                                            ...
                                                            <?php if(has_post_thumbnail()){ ?>
                                                                ...
                                                                <div class="post-thumbnail">
                                                                    <a href="<?php the_permalink(); ?>">
                                                                        <?php the_post_thumbnail($tab_with_carousel_thumb_size); ?>
                                                                    </a>
                                                                </div>
                                                                <!-- End Post Thumbnail  -->
                                                            <?php } ?>
...
Plugin::instance()->widgets_manager->register_widget_type( new Blogar_Elementor_Widget_Tab_with_Carousel() );
