...
class WC_Functions {
	...
	public function __construct() {
		...
		/* Title */
		add_filter( 'wooctheme_page_title',                              array( $this, 'page_title' ) );

		add_action( 'wp_ajax_uiart_product_search_autocomplete',        array( $this, 'product_search_autocomplete') );
		add_action( 'wp_ajax_nopriv_uiart_product_search_autocomplete', array( $this, 'product_search_autocomplete') );
		...
		if ( function_exists( 'YITH_WCWL_Frontend' ) && class_exists( 'YITH_WCWL_Ajax_Handler' )  ) {
			...
			add_action( 'wp_ajax_uiart_add_to_wishlist',                array( $this, 'add_to_wishlist' ) );
			add_action( 'wp_ajax_nopriv_uiart_add_to_wishlist',         array( $this, 'add_to_wishlist' ) );
		}
	...
	// Single
	public function wooc_single_layout() {		
		$meta       = !empty( get_post_meta( get_the_id(), 'wc_single_layout', true ) ) ? get_post_meta( get_the_id(), 'wc_single_layout', true ) : 'default';
		$op_layout  = WOOCTheme::$options['product_wc_single_layout'];
	...
	public function body_classes( $classes ) {
		...
		if ( is_singular( 'product' ) ) {
			$classes[] = 'single-product-layout-' . WOOCTheme::$options['product_wc_single_layout'];			
		}
	...
	public function product_search_autocomplete() {
		if ( isset( $_REQUEST['term'] ) ) {
			...
			$query = apply_filters( 'uiart_product_search_query', $query, $_REQUEST );
			...
			$result = apply_filters( 'uiart_product_search_query_result', $result, $_REQUEST );
			wp_send_json( $result );
		}
	}	
	...
	public function pagination() {
		...
		else if ( WOOCTheme::$options['wc_pagination'] == 'infinity-scroll' ) {
	...
	public function star_rating_html( $html, $rating, $count ){
		$html = 0 < $rating ?'<div class="wooctheme-star-rating"><span style="width:' . ( ( $rating / 5 ) * 100 ) . '%"></span></div>' : '';
		return $html;
	}
	...
	public function shop_topbar() {
		if ( isset(WOOCTheme::$options['shop_header'])){
	...
	public function loop_shop_per_page(){
		return WOOCTheme::$options['wc_num_product'];
	}

	public function loop_shop_columns(){
		if ( WOOCTheme::$layout == 'full-width' ) {
			return 4;
		}
		return 3;
	}
...
public function sale_flash( $args, $post, $product  ) {
	...
	if ( WOOCTheme::$options['wc_sale_label'] == 'percentage' ) {
	    ...
	    if ( $product->get_type() === 'variable' ) {
	        ...
	        // Return variation sale percent     
 			$poff = esc_html( 'OFF', 'uiart' );
			return sprintf('<span class="onsale">%s%% %s</span>', $highest_sale_percent , $poff );				       
	        ...
	        $poff = esc_html( 'OFF', 'uiart' );
			return sprintf('<span class="onsale">%s%% %s</span>', $sale_percent , $poff );	
			...
			$poff = esc_html( 'OFF', 'uiart' );
			return sprintf('<span class="onsale">%s%% %s</span>', $discount , $poff );	
	...
	public function add_wc_button_wrapper_start(){
		echo '<div class="single-woocwc-wrapper">';
	}

	public function add_wc_button_wrapper_end(){
		echo '</div>';
	}

	public function single_add_to_cart_button(){
		echo '<div class="product-single-wc-wrp">';
		self::print_add_to_wishlist_icon();	
		self::print_compare_icon();
		echo '</div>';
	}

/*	public function single_wishlist_quickview_(){
	...
	public function single_product_layout_hooks() {
		...
		switch ( WOOCTheme::$options['product_wc_single_layout'] ) {
			case '2':

				remove_action( 'woocommerce_single_product_summary',       'woocommerce_template_single_rating', 10 );
				
			    add_action( 'woocommerce_single_product_summary',           		array( $this, 'add_wc_button_wrapper_start' ), 34 );
				add_action( 'woocommerce_single_product_summary',            		array( $this, 'single_add_to_cart_button' ), 35 );
				add_action( 'woocommerce_single_product_summary',          			array( $this, 'single_1_social' ), 36);

				add_action( 'woocommerce_single_product_summary',  'woocommerce_template_single_rating' , 37);

				add_action( 'woocommerce_single_product_summary',            		array( $this, 'add_wc_button_wrapper_end' ), 38 );
				
				remove_action( 'woocommerce_single_product_summary',       'woocommerce_template_single_meta', 40 );
				add_action( 'woocommerce_single_product_summary',     'woocommerce_template_single_meta', 40 );
				
				remove_action( 'woocommerce_after_single_product_summary', 			'woocommerce_output_product_data_tabs', 10 );
				//add_action( 'woocommerce_after_single_product_summary',          	array( $this, 'single_3_tabs_wrp_start' ), 10 );
				add_action( 'woocommerce_after_single_product_summary',          	'woocommerce_output_product_data_tabs', 15 );
				//add_action( 'woocommerce_after_single_product_summary',          	array( $this, 'single_3_tabs_wrp_end' ), 20 );
			...
			case '3':
				if(WOOCTheme::$options['woocommerce_header']){
					remove_action( 'woocommerce_archive_description',              'woocommerce_breadcrumb', 10 );				
					add_action( 'woocommerce_before_main_content',          array( $this, 'single_3_tabs_wrp_start' ), 10 );
					add_action( 'woocommerce_before_main_content',              'woocommerce_breadcrumb', 20 );
					add_action( 'woocommerce_before_main_content',          array( $this, 'single_3_tabs_wrp_end' ), 30 );
				}

				remove_action( 'woocommerce_single_product_summary',       'woocommerce_template_single_meta', 40 );
				add_action( 'woocommerce_after_single_product_summary',     'woocommerce_template_single_meta', 5 );

				add_action( 'woocommerce_single_product_summary',           	array( $this, 'add_wc_button_wrapper_start' ), 40 );
				add_action( 'woocommerce_single_product_summary',            	array( $this, 'single_add_to_cart_button' ),  41 );
				add_action( 'woocommerce_single_product_summary',          		array( $this, 'single_1_social' ), 42 );
				add_action( 'woocommerce_single_product_summary',            	array( $this, 'add_wc_button_wrapper_end' ), 43 );
	...
	public function single_3_tabs_wrp_start(){
	echo '<div class="container-wrp">';
		echo '<div class="container">';
	}

	public function single_3_tabs_wrp_end(){
		echo '</div>';
	echo '</div>';
	}
	...
	public function single_gallery_img_size( $args ) {
		if ( class_exists( 'WooProductVariationGalleryPro' ) && is_product() ) {
			if ( WOOCTheme::$options['product_wc_single_layout'] == '3' ) {
				return 'wooctheme-size7';
			}
		}
		return $args;
	}
	...
	public function show_or_hide_related_products(){
		...
		if ( empty( WOOCTheme::$options['wc_related'] ) ) {
			remove_action( 'woocommerce_after_single_product_summary', 'woocommerce_output_related_products', 20 );
		}
	}

	public function single_1_social(){ ?>		
		<?php if ( WOOCTheme::$options['wc_social'] ): ?>
		<div class="product-social">
			<span class="product-social-title"><?php esc_html_e( 'Share:', 'uiart' );?></span>
			<ul class="product-social-items">
				<?php foreach ( WC_Functions::social_sharing() as $key => $sharer ): ?>
					<li class="social-<?php echo esc_attr( $key );?>"><a href="<?php echo esc_url( $sharer['url'] );?>" target="_blank"><i class="fab <?php echo esc_attr( $sharer['icon'] );?>"></i></a></li>
				<?php endforeach; ?>
			</ul>
		</div>
	<?php endif; 
	...
	public function single_3_stock() {
		...
		<div class="single_3-avaibility">
			<span class="woocue-title"><?php esc_html_e( 'Avaibility', 'uiart' ); ?>:</span>
			<span class="woocue-stock"><?php echo esc_html( self::get_stock_status() ); ?></span>
		</div>
		<?php
	}
	...
	public function hide_product_data_tab( $tabs ){
		if ( empty( WOOCTheme::$options['wc_description'] ) ) {
			unset( $tabs['description'] );
		}
		if ( empty( WOOCTheme::$options['wc_reviews'] ) ) {
			unset( $tabs['reviews'] );
		}
		if ( empty( WOOCTheme::$options['wc_additional_info'] ) ) {
	...
	public function product_review_form( $comment_form ){
		...
		$comment_form['fields'] = array(
			'author' => '<div class="row"><div class="col-sm-6"><div class="comment-form-author form-group"><input id="author" name="author" type="text" class="form-control" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30" placeholder="' . esc_html__( 'Name *', 'uiart' ) . '" required /></div></div>',
			'email'  => '<div class="comment-form-email col-sm-6"><div class="form-group"><input id="email" class="form-control" name="email" type="email" value="' . esc_attr(  $commenter['comment_author_email'] ) . '" size="30" placeholder="' . esc_html__( 'Email *', 'uiart' ) . '" required /></div></div></div>',
		);

		$comment_form['comment_field'] = '';

		if ( get_option( 'woocommerce_enable_review_rating' ) === 'yes' ) {
			$comment_form['comment_field'] = '<p class="comment-form-rating"><label for="rating">' . esc_html__( 'Your Rating', 'uiart' ) .'</label>
			<select name="rating" id="rating" required>
			<option value="">' . esc_html__( 'Rate&hellip;', 'uiart' ) . '</option>
			<option value="5">' . esc_html__( 'Perfect', 'uiart' ) . '</option>
			<option value="4">' . esc_html__( 'Good', 'uiart' ) . '</option>
			<option value="3">' . esc_html__( 'Average', 'uiart' ) . '</option>
			<option value="2">' . esc_html__( 'Not that bad', 'uiart' ) . '</option>
			<option value="1">' . esc_html__( 'Very Poor', 'uiart' ) . '</option>
			</select></p>';
		}

		$comment_form['comment_field'] .= '<div class="form-group comment-form-comment"><textarea id="comment" name="comment" class="form-control" placeholder="' . esc_html__( 'Your Review *', 'uiart' ) . '" cols="45" rows="8" required></textarea></div>';
	...
	public function show_or_hide_cross_sells(){
		...
		if ( !empty( WOOCTheme::$options['wc_cross_sell'] ) ) {
	...
	public static function print_quickview_icon( $icon = true, $text = false ){
		...
		if ( is_shop() && !WOOCTheme::$options['wc_shop_quickview_icon'] ) {
			return false;
		}

		if ( is_product() && !WOOCTheme::$options['wc_product_quickview_icon'] ) {
			return false;
		}
		...
		if ( $text ) {
			$html .= '<span>' . esc_html__( 'QuickView', 'uiart' ) . '</span>';
		}
  		$allowed_tags = wp_kses_allowed_html( 'post' ); 
		?>
		<a href="#" data-toggle="tooltip" data-placement="top" class="yith-wcqv-button" data-product_id="<?php echo esc_attr( $product->get_id() );?>" title="<?php esc_attr_e( 'QuickView', 'uiart' ); ?>"><?php echo wp_kses( $html, $allowed_tags ); ?></a>
		<?php
        return true;
	}
	...
	public static function print_compare_icon( $icon = true, $text = false ){
		...
		if ( is_shop() && !WOOCTheme::$options['wc_shop_compare_icon'] ) {
			return false;
		}

		if ( is_product() && !WOOCTheme::$options['wc_product_compare_icon'] ) {
			return false;
		}
		...
		if ( $text ) {
			$html .= '<span>' . esc_html__( 'Compare', 'uiart' ) . '</span>';
		}
		$allowed_tags = wp_kses_allowed_html( 'post' );
		?>
		<a href="<?php echo esc_url( $url );?>" data-toggle="tooltip" class="compare" data-product_id="<?php echo esc_attr( $id );?>" title="<?php esc_attr_e( 'Add To Compare', 'uiart' ); ?>"><?php echo wp_kses( $html, $allowed_tags ); ?></a> 
		<?php
        return true;
	}
	...
	public static function print_add_to_wishlist_icon( $icon = true, $text = false ){
		...
		if ( is_shop() && !WOOCTheme::$options['wc_shop_wishlist_icon'] ) {
			return false;
		}

		if ( is_product() && !WOOCTheme::$options['wc_product_wishlist_icon'] ) {
			return false;
		}
	...
	public static function social_sharing() {
		...
		foreach ( WOOCTheme::$options['wc_share'] as $key => $value ) {
			if ( !$value ) {
				unset( $sharer[$key] );
			}
		}
	...
	public function add_to_wishlist() {
		check_ajax_referer( 'uiart_wishlist_nonce', 'nonce' );
...
*/
	...
	public static function get_stock_status() {
		global $product;
		return $product->is_in_stock() ? esc_html__( 'In Stock', 'uiart' ) : esc_html__( 'Out of Stock', 'uiart' );
	}
	...
	public static function uiart_wc_format_stock_for_display( $product ) {		
		
		$display      = __( 'In stock', 'uiart' );
		...
		switch ( get_option( 'woocommerce_stock_format' ) ) {
			case 'low_amount':
				if ( $stock_amount <= get_option( 'woocommerce_notify_low_stock_amount' ) ) {
					...
					$display = sprintf( __( 'Only %s left in stock', 'uiart' ), wc_format_stock_quantity_for_display( $stock_amount, $product ) );
				}
				break;
			case '':
				/* translators: %s: stock amount */
				$display = sprintf( __( '%s in stock', 'uiart' ), wc_format_stock_quantity_for_display( $stock_amount, $product ) );
				break;
		}

		if ( $product->backorders_allowed() && $product->backorders_require_notification() ) {
			$display .= ' ' . __( '(can be backordered)', 'uiart' );
		}
        return $display_html   = '<span class="product-meta-title">'. esc_html_e( "Avaibility", "uiart" ) . ' : </span><span class="product-meta-content sku">' . $display .'<span>';
 ...
 */
    ...
    public static  function wooc_category_menu() {
        ...
        $hide_empty = ( WOOCTheme::$options['shop_categories_hide_empty'] ) ? true : false;

        // Should top-level categories be displayed?
        if ( WOOCTheme::$options['shop_categories_top_level'] == '0' && $is_category ) {
 ...
 */
   ...
   public static  function wooc_category_menu_output( $is_category, $current_cat_id, $hide_empty ) {
        ...
        $output_cat = '<li' . $all_categories_class . '><a href="' . esc_url ( $page_url ) . '">' . esc_html__( 'All', 'uiart' ) . '</a></li>';
        $output_sub_cat = '';
        ...
        $orderby = 'slug';
        $order = 'ASC';
        if ( isset( WOOCTheme::$options['shop_categories_orderby'] ) ) {
            $orderby = WOOCTheme::$options['shop_categories_orderby'];
            $order = WOOCTheme::$options['shop_categories_order'];
        }
   ...
   public static  function wooc_sub_category_menu_back_link( $url, $categories_menu_divider, $class = '' ) {
        return '<li class="wooc-category-back-button' . esc_attr( $class ) . '"><a href="' . esc_url( $url ) . '"><i class="wooc-font wooc-font-arrow-left"></i> ' . esc_html__( 'Back', 'uiart' ) . '</a>' . $categories_menu_divider . '</li>';
    }
   ...
   public static  function wooc_sub_category_menu_output( $current_cat_id, $hide_empty ) {
        ...
        // "Back" link
        $output_back_link = '';
        if ( WOOCTheme::$options['shop_categories_back_link'] ) {
            ...
            } else if ( WOOCTheme::$options['shop_categories_back_link'] == '1st' ) {
