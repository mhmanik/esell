...
$sku              = $product->get_sku();
$sku              = $sku? $sku : esc_html__( 'N/A', 'uiart' );
$stock_status     = WC_Functions::get_stock_status();
$cats_title       = _n( 'Category', 'Categories', count( $product->get_category_ids() ) , 'uiart' );
$tags_title       = _n( 'Tag', 'Tags', count( $product->get_tag_ids() ) , 'uiart' );
$cats_html        = wc_get_product_category_list( $product->get_id() );
$tags_html        = wc_get_product_tag_list( $product->get_id() );
$has_cats_n_tags  = WOOCTheme::$options['wc_cats'] && $cats_html && WOOCTheme::$options['wc_tags'] && $tags_html ? true : false;
$has_cats_or_tags = ( WOOCTheme::$options['wc_cats'] && $cats_html ) || ( WOOCTheme::$options['wc_tags'] && $tags_html ) ? true : false;
$sku 			  = $product->get_sku() ? $sku : esc_html__( 'N/A', 'woocommerce' ); 
$layout = WOOCTheme::$options['product_wc_single_layout'] ? WOOCTheme::$options['product_wc_single_layout'] : '1';
if ($layout == '1') { ?>
...
<?php } elseif($layout == '2') { ?>
	<div class="product-meta-area-wrp">
		<div class="product_meta-area product_meta-area-js">
			<?php do_action( 'woocommerce_product_meta_start' ); ?>
			<?php if ( WOOCTheme::$options['in_stock_avaibility'] ) : ?>
				<div class="product-meta-avaibility">
				    <?php echo WC_Functions::uiart_wc_format_stock_for_display($product) ?>	
				</div>
				<?php endif; ?>					
				...
				<?php if ( WOOCTheme::$options['wc_cats'] && $cats_html ): ?>
				...
				<?php endif; ?>	
				<?php if ( WOOCTheme::$options['wc_tags'] && $tags_html ): ?>
					<div class="product-meta-term">
