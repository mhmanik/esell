...
class Wooc_product_aj_search {
    ...
    public function wooc_sratch_ajax_responder($data = '') {
        ...
        $search_limit = isset( $axil_options['wooc_search_auto_suggest_limit'])
                && !empty( $axil_options['wooc_search_auto_suggest_limit'])? (int) $axil_options['wooc_search_auto_suggest_limit'] :  10;
        ...
        foreach ($posts as $post):
            ...
            if(isset( $axil_options['wooc_search_img_status']) &&  $axil_options['wooc_search_img_status']){
        ...
        header('Content-Type: application/json');
         echo sprintf(__('%s','esell'),$search_output) ;
        die();
    }

    public function wooc_product_search_callback() {
        $axil_options           = Helper::axil_get_options();
        if(isset( $axil_options['wooc_search_auto_suggest_status'])
                      ...
                      input.on('keyup change', function() {
                        ...
                        if (term.length > 2 && term != $(this).attr('data-old-term')) {
                            if(subm.hasClass('icon-search')){
                                subm.removeClass('icon-search').addClass('fa fa-spinner');
                            }
                          ...
                          currentAjaxRequest = $.getJSON(searchURL + '&view=json', function(data) {
                            ...
                            subm.removeClass('fa fa-spinner').addClass('icon-search');
                            ...
                            if(data.results_count == 0) {
                              ...
                              if(data.results_count > 10) {
                                resultsList.append('<li><span class="title"><a href="' + searchURL + '"><?php esc_html_e('See all results', 'esell')?> (' + data.results_count + ')</a></span></li>');
                              }
