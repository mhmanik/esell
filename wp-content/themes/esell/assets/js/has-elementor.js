	...
	var Theme = {
		...
		wooc_toltp: function wooc_toltp() {
			$(document).on('mouseover', '.wooc-toltp', function() {
				...
				$tooltip = '<div class="esell-tooltip">' + '<div class="esell-tooltip-content">' + tips + '</div>' + '<div class="esell-tooltip-bottom"></div>' + '</div>';
				$('body').append($tooltip);
				var $tooltip = $('body > .esell-tooltip');
				var tHeight = $tooltip.outerHeight();
				var tBottomHeight = $tooltip.find('.esell-tooltip-bottom').outerHeight();
		...
		wooc_logo_slider: function wooc_logo_slider() {
			var logoSlickCarousel = $('.esell-logo-slider');
			...
			if ($.fn.autocomplete) {
				$(".ps-autocomplete-js .product-autocomplete-js").autocomplete({
					...
					source: function source(req, response) {
						req.action = 'esell_product_search_autocomplete';
						$.ajax({
							dataType: "json",
							type: "POST",
		...
		countdown: function countdown() {
			if (typeof $.fn.countdown == 'function') {
				try {
					var day = EsellObj.day == 'Day' ? 'Day%!D' : EsellObj.day,
						hour = EsellObj.hour == 'Hour' ? 'Hour%!D' : EsellObj.hour,
						minute = EsellObj.minute == 'Minute' ? 'Minute%!D' : EsellObj.minute,
						second = EsellObj.second == 'Second' ? 'Second%!D' : EsellObj.second;
