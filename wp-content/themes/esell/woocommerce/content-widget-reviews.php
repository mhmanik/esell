	...
	<div class="woocue-content">
		<div class="woocue-left">
			<a href="<?php echo esc_url( $product->get_permalink() ); ?>">
				...
				<div class="woocue-icon"><i class="flaticon-plus-symbol"></i></div>
			</a>
		</div>
		<div class="woocue-right">
			<a class="product-title" href="<?php echo esc_url( $product->get_permalink() ); ?>">			
				<?php echo wp_kses( $product->get_name(), 'alltext_allow' );?>
			</a>			
			<?php echo wc_get_rating_html( intval( get_comment_meta( $comment->comment_ID, 'rating', true ) ) );?>
			<span class="reviewer"><?php echo sprintf( esc_html__( 'by %s', 'esell' ), get_comment_author( $comment->comment_ID ) ); ?></span>
		</div>
	</div>
