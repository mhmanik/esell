...
class WC_Functions {
	...
	public function __construct() {
		...
		add_action( 'wp_ajax_esell_product_search_autocomplete',        array( $this, 'product_search_autocomplete') );
		add_action( 'wp_ajax_nopriv_esell_product_search_autocomplete', array( $this, 'product_search_autocomplete') );
		...
		if ( function_exists( 'YITH_WCQV_Frontend' ) ) {
			...
			remove_action( 'woocommerce_after_shop_loop_item',         array( YITH_WCQV_Frontend::get_instance(), 'yith_add_quick_view_button' ), 99 );
			remove_action( 'yith_wcwl_table_after_product_name',       array( YITH_WCQV_Frontend::get_instance(), 'yith_add_quick_view_button' ), 99 );

			remove_action( 'yith_proteo_products_loop_add_to_cart_actions', 'yith_add_quick_view_button', 15 );
		...
		if ( function_exists( 'YITH_WCWL_Frontend' ) && class_exists( 'YITH_WCWL_Ajax_Handler' )  ) {
			...
			add_action( 'wp_ajax_esell_add_to_wishlist',                array( $this, 'add_to_wishlist' ) );
			add_action( 'wp_ajax_nopriv_esell_add_to_wishlist',         array( $this, 'add_to_wishlist' ) );
		}
	...
	public function wooc_single_layout() {	
		...
		$op_layout  = $axil_options['product_wc_single_layout'];
	...
	public function body_classes( $classes ) {
		...
		if ( is_singular( 'product' ) ) {
			$classes[] = 'single-product-layout-' . $axil_options['product_wc_single_layout'];			
		}
	...
	public function product_search_autocomplete() {
		if ( isset( $_REQUEST['term'] ) ) {
			...
			$query = apply_filters( 'esell_product_search_query', $query, $_REQUEST );
			...
			$result = apply_filters( 'esell_product_search_query_result', $result, $_REQUEST );
			wp_send_json( $result );
		}
	}	
	...
	public function pagination() {
		...
		else if ( $axil_options['wc_pagination'] == 'infinity-scroll' ) {
	...
	public function shop_topbar() {
			$axil_options           = Helper::axil_get_options();
		if ( isset($axil_options['shop_header'])){
	...
	public function loop_shop_columns(){
		$axil_options  = Helper::axil_get_options();
		//if ( $axil_options['layout'] == 'full-width' ) {
			return 4;
		//}
		return 3;
	}
...
$axil_options  = Helper::axil_get_options();
	if ( $axil_options['wc_sale_label'] == 'percentage' ) {
	    ...
	    if ( $product->get_type() === 'variable' ) {
	        ...
	        // Return variation sale percent     
 			$poff = esc_html( 'OFF', 'esell' );
			return sprintf('<div class="label-block label-right"><div class="product-badget">%s%% %s</div></div>', $highest_sale_percent , $poff );				       
	        ...
	        $poff = esc_html( 'OFF', 'esell' );
			return sprintf('<div class="label-block label-right"><div class="product-badget">%s%% %s</div></div>', $sale_percent , $poff );	
			...
			$poff = esc_html( 'OFF', 'esell' );
			return sprintf('<div class="label-block label-right"><div class="product-badget">%s%% %s</div></div>', $discount , $poff );	
...
$axil_options  = Helper::axil_get_options();
	...
	public function add_to_cart_button_wrapper_start(){
		echo '<div class="product-action-wrapper d-flex-center">';
	}
...
/*	public function single_wishlist_quickview_(){
	...
	public function single_product_layout_hooks() {
		...
		switch ( $axil_options['product_wc_single_layout'] ) {
			case '2':
			...
			case '3':
				if($axil_options['woocommerce_header']){
			...
			default:
				...
				//add_action( 'woocommerce_single_product_summary',          array( $this, 'single_3_stock' ), 15 );
	...
	echo '<div class="woocommerce-tabs wc-tabs-wrapper bg-gradient-1">';
	...
	public function single_gallery_img_size( $args ) {
		...
		if ( class_exists( 'WooProductVariationGalleryPro' ) && is_product() ) {
			if ( $axil_options['product_wc_single_layout'] == '3' ) {
				return 'axiltheme-size7';
			}
		}
		return $args;
	}
	...
	public function single_1_social(){ 
		...
		<div class="product-social">
			<span class="product-social-title"><?php esc_html_e( 'Share:', 'esell' );?></span>
	...
	public function single_3_stock() {
		...
		<div class="single_3-avaibility">
			<span class="woocue-title"><?php esc_html_e( 'Avaibility', 'esell' ); ?>:</span>
	...
	public function product_review_form( $comment_form ){
		...
		$comment_form['fields'] = array(
			'author' => '<div class="row"><div class="col-sm-6"><div class="comment-form-author form-group"><input id="author" name="author" type="text" class="form-control" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30" placeholder="' . esc_html__( 'Name *', 'esell' ) . '" required /></div></div>',
			'email'  => '<div class="comment-form-email col-sm-6"><div class="form-group"><input id="email" class="form-control" name="email" type="email" value="' . esc_attr(  $commenter['comment_author_email'] ) . '" size="30" placeholder="' . esc_html__( 'Email *', 'esell' ) . '" required /></div></div></div>',
		);

		$comment_form['comment_field'] = '';

		if ( get_option( 'woocommerce_enable_review_rating' ) === 'yes' ) {
			$comment_form['comment_field'] = '<p class="comment-form-rating"><label for="rating">' . esc_html__( 'Your Rating', 'esell' ) .'</label>
		...
		$comment_form['comment_field'] .= '<div class="form-group comment-form-comment"><textarea id="comment" name="comment" class="form-control" placeholder="' . esc_html__( 'Your Review *', 'esell' ) . '" cols="45" rows="8" required></textarea></div>';
	...
	public static function print_quickview_icon( $icon = true, $text = false ){
		...
		if ( is_shop() && !$axil_options['wc_shop_quickview_icon'] ) {
			return false;
		}

		if ( is_product() && !$axil_options['wc_product_quickview_icon'] ) {
			return false;
		}
		...
		if ( $text ) {
			$html .= '<span>' . esc_html__( 'QuickView', 'esell' ) . '</span>';
		}
  		$allowed_tags = wp_kses_allowed_html( 'post' ); 
		?>
		<a href="#" data-toggle="tooltip" data-placement="top" class="yith-wcqv-button" data-product_id="<?php echo esc_attr( $product->get_id() );?>" title="<?php esc_attr_e( 'QuickView', 'esell' ); ?>"><?php echo wp_kses( $html, $allowed_tags ); ?></a>
		<?php
        return true;
	}
	...
	public static function print_compare_icon( $icon = true, $text = false ){
		...
		if ( is_shop() && !$axil_options['wc_shop_compare_icon'] ) {
			return false;
		}
		...
		if ( $text ) {
			$html .= '<span>' . esc_html__( 'Compare', 'esell' ) . '</span>';
		}
		$allowed_tags = wp_kses_allowed_html( 'post' );
		?>
		<a href="<?php echo esc_url( $url );?>" data-toggle="tooltip" class="axil-btn btn-outline btn-icon compare" data-product_id="<?php echo esc_attr( $id );?>" title="<?php esc_attr_e( 'Add To Compare', 'esell' ); ?>"><?php echo wp_kses( $html, $allowed_tags ); ?></a> 
		<?php
        return true;
	}
	...
	public static function print_add_to_wishlist_icon( $icon = true, $text = false ){
		...
		if ( is_shop() && !$axil_options['wc_shop_wishlist_icon'] ) {
			return false;
		}

		if ( is_product() && !$axil_options['wc_product_wishlist_icon'] ) {
			return false;
		}
	...
	public function add_to_wishlist() {
		check_ajax_referer( 'esell_wishlist_nonce', 'nonce' );
...
*/
	...
	public static function get_stock_status() {
		global $product;
		return $product->is_in_stock() ? esc_html__( 'In Stock', 'esell' ) : esc_html__( 'Out of Stock', 'esell' );
	}
	...
	public static function esell_wc_format_stock_for_display( $product ) {		
		
		$display      = __( 'In stock', 'esell' );
		...
		switch ( get_option( 'woocommerce_stock_format' ) ) {
			case 'low_amount':
				if ( $stock_amount <= get_option( 'woocommerce_notify_low_stock_amount' ) ) {
					...
					$display = sprintf( __( 'Only %s left in stock', 'esell' ), wc_format_stock_quantity_for_display( $stock_amount, $product ) );
				}
				break;
			case '':
				/* translators: %s: stock amount */
				$display = sprintf( __( '%s in stock', 'esell' ), wc_format_stock_quantity_for_display( $stock_amount, $product ) );
				break;
		}

		if ( $product->backorders_allowed() && $product->backorders_require_notification() ) {
			$display .= ' ' . __( '(can be backordered)', 'esell' );
		}
        return $display_html   = '<span class="product-meta-title">'. esc_html_e( "Avaibility", "esell" ) . ' : </span><span class="product-meta-content sku">' . $display .'<span>';
 ...
 */
   ...
   public static  function wooc_category_menu_output( $is_category, $current_cat_id, $hide_empty ) {
        ...
        $output_cat = '<li' . $all_categories_class . '><a href="' . esc_url ( $page_url ) . '">' . esc_html__( 'All', 'esell' ) . '</a></li>';
        $output_sub_cat = '';
   ...
   public static  function wooc_sub_category_menu_back_link( $url, $categories_menu_divider, $class = '' ) {
        return '<li class="wooc-category-back-button' . esc_attr( $class ) . '"><a href="' . esc_url( $url ) . '"><i class="wooc-font wooc-font-arrow-left"></i> ' . esc_html__( 'Back', 'esell' ) . '</a>' . $categories_menu_divider . '</li>';
    }
