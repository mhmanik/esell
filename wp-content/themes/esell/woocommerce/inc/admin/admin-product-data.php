    ...
    public function product_attribute_settings( $attribute, $i ) {
        ...
        if ( isset( $attribute['variation'] ) && $attribute['variation'] == true ) {
            ...
            if ( in_array( $attr->attribute_type, $custom_attr_types ) ) {
                ...
                <tr>
                    <td>
                        <label><input type="checkbox" class="checkbox" <?php checked( $checked, true ); ?> name="wooc_attribute_catalog_visibility[<?php echo esc_attr( $attribute['name'] ); ?>]" value="1" /> <?php esc_html_e( 'Show in product catalog', 'esell' ); ?></label>
                    </td>
                </tr>
                <?php
            }
